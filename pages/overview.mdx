...
Optimimstic rollups assume that all transactions are valid by default, hence the "optimism".
However, any party can submit a fraud proof to prove that a state transition in the chain is invalid.
Transctions than involve L2 to L1 communication (including withdrawals) need to wait for the dispute window (~7 days) to pass before being finalized.
